...
begin
	...
	urlExt   = "BABA"
	probName = "BABA Strings"
	origName = "BABA Strings"
	
	filenameBase = "baba"
	fileIDRange = 1:30 #nothing #1:12 # ← nothing
	fileIDPad = 3
...
# ╔═╡ 5e4081db-0e56-4a12-936d-1e5c7dda66fb
...
function parseDataset(raw)
	...
	numBlimps, xₘ, yₘ = [parse(Int, subStr)    for subStr in split(first(raw))]

	
	blimpLocs =  Vector{NamedTuple}(undef, numBlimps)

	for b in 1:numBlimps
		x,y =  [parse(Int, subStr)   for subStr in split(raw[1+b])]
		blimpLocs[b] = (x=x, y=y)
	end
 
	(mirrorLoc = (x=xₘ,  y=yₘ),    blimpLocs = blimpLocs)
end
...
=#
function canSeeDirectly(mirrorLoc, blimpLoc)

	if blimpLoc.x < mirrorLoc.x;     return true;    end

	if blimpLoc.y  // blimpLoc.x  >  
	   mirrorLoc.y // mirrorLoc.x
...
# ╔═╡ 8f60a8c6-f064-48ab-a68d-a6e7af5e4102


# ╔═╡ 83776c7c-eddf-4fc0-b0d2-a1567043af4a
#=
	The image if visible IFF either is true:

		The blimp is to the left of the mirror and no higher than the top

		The blimp is to the left of the mirror and the angle upward from 
	 	the top to the blimp is no more than the angle downward to the origin


	❗ We do not need to solve for the reflection position on the mirror

=#

function canSeeImage(mirrorLoc, blimpLoc)

	if blimpLoc.x > mirrorLoc.x;    return false;    end

	if blimpLoc.y ≤ mirrorLoc.y;    return true;    end

 
	slopeUpFromMirrorTopToBlimp = (blimpLoc.y - mirrorLoc.y)  //
	                              (mirrorLoc.x - blimpLoc.x)

	
	
	slopeDownFromMirrorToToOrigin = mirrorLoc.y // mirrorLoc.x

 

	if slopeUpFromMirrorTopToBlimp ≤ slopeDownFromMirrorToToOrigin
		return true
	end
...
# ╔═╡ 4f4183e0-a534-4773-9e47-5d8505591daf
canSeeImage((x=12, y=4), (x=10, y=5))

# ╔═╡ 91c122c8-05b7-4193-865a-426b858ed113
@htl("""<div class = "doc">

	Do we need to solve for the reflection position on the mirror?
</div>""")
...
# ╔═╡ d92ce17b-0d94-4526-ac31-2b5c958ff835
#=
...
function sln(dset)
	...
	mirrorLoc, blimpLocs = dset

	for blimpLoc in blimpLocs
		if canSeeDirectly(mirrorLoc, blimpLoc);    res += 1;    end
		if canSeeImage(mirrorLoc, blimpLoc);       res += 1;    end
	end
...
# ╔═╡ 353caf47-f099-4581-b933-06f0170a29f4
function numCanSeeDirectly(dset)
 	res = 0
...
# ╔═╡ 58e7577d-5168-4940-95bb-8fd6069b8492
function numCanSeeImage(dset)
 	res = 0
...
urlExnOnePerLine = "https://raw.githubusercontent.com/" * 
                   "Tim-Gaede/UCF_HSPT_OE2_2024_Blimps/main/blimps.out"

# ╔═╡ 4a8ea51b-6eb2-44c8-95ab-23e0f42737f7
exns = GaGa.vecStrFrom_url(urlExnOnePerLine)

# ╔═╡ c8588977-913e-4310-8436-019338ed8d5e
exns
...
# ╔═╡ 3a1eb9e4-779f-4e8f-a96a-7de4e40e1c00
dsets[1]

# ╔═╡ 1d4cbcc2-be31-4437-92d1-c026d0dd8e6f
numCanSeeDirectly(dsets[1])

# ╔═╡ 4c5bbbb0-5bd2-4024-aa64-6e31398bc709
numCanSeeImage(dsets[1])

# ╔═╡ b5bb3590-783b-41c5-a424-0ce83069f6c7
slns = [sln(dsets[d])    for d in ei(dsets)]

# ╔═╡ fc31f119-08e9-4c8f-a0c9-4b65624e560b
slns == exns
...
# ╔═╡ e539aabb-1253-4667-b7c0-608c4ba8ca72
[(slns[d], exns[d])    for d in ei(dsets)]

# ╔═╡ cb6044f4-e406-4625-8bda-4774564f60fe
veracityVector = [slns[d] == exns[d]    for d in ei(dsets)]

# ╔═╡ 7c02d5c7-9d0f-4bac-a4af-d4d0e90cc293
all(veracityVector)
